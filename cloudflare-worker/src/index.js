...
function answeredQuestionPanel(site, title) {
  const sourceNames = (site.sources || []).map((source) => source.name).join(", ");
  const sourceAnswer = sourceNames
    ? `The published fields come from ${sourceNames}. Open Sources & method for the release, rights and transformation boundary.`
    : "The page uses only the fields admitted to the approved public projection. Open Sources & method for the current source and publication boundary.";
  const examples = (site.examples || []).slice(0, 4).join(", ").toLowerCase();
  return `<section class="question-panel" aria-labelledby="questions-this-page-answers"><p class="eyebrow">Questions this evidence can answer</p><h2 id="questions-this-page-answers">Use the record, then check its limits</h2><div class="question-grid">
    <details open><summary>What exactly does this page identify?</summary><p>It is the ${esc(site.entity)} evidence page for <strong>${esc(title)}</strong>. Use the displayed identifier and source link to distinguish it from similarly named records.</p></details>
    <details><summary>Which source supports the answer?</summary><p>${esc(sourceAnswer)}</p></details>
    <details><summary>When was the evidence collected and rechecked?</summary><p>Record-level observation, action, reporting or snapshot dates appear beside the relevant facts. The footer separately states when the public evidence was rechecked.</p></details>
    <details><summary>What can I learn from this record?</summary><p>Where supported, this product explains ${esc(examples || site.description.toLowerCase())}. Missing values are left unavailable rather than inferred.</p></details>
    <details><summary>What should I check before acting?</summary><p>Read the limitation beside the evidence and open the controlling official source for a time-sensitive, legal, safety, financial or eligibility decision. You can also report a correction.</p></details>
  </div></section>`;
}
...
function tideEntityPage(site, entity, origin, id) {
  ...
  <div class="notice"><strong>Evidence dates:</strong> source collected at the observation and prediction times shown above; evidence rechecked when the approved projection was generated ${esc(prettyTime(entity._projectionGeneratedAt))}.</div>
...
function charityEntityPage(site, entity, origin, id) {
  ...
  <div class="notice"><strong>Evidence dates:</strong> source collected in the Charity Commission extract dated ${esc(prettyDate(entity.date_of_extract))}; evidence rechecked when the approved projection was generated ${esc(prettyTime(entity._projectionGeneratedAt))}.</div>
...
function fccEntityPage(site, entity, origin, id) {
  ...
  return shell(
    ...
    <div class="notice"><strong>Freshness and privacy:</strong> source collected through the latest action on ${esc(prettyDate(entity.latest_action_date))}; evidence rechecked when the approved projection was generated ${esc(prettyTime(entity._projectionGeneratedAt))}. It excludes applicant addresses, contacts, signatures, email addresses and private exhibits.</div>
    <details class="technical"><summary>Complete approved source fields</summary><div class="technical-grid">${Object.entries(entity).filter(([key, value]) => !key.startsWith("_") && value !== null && value !== undefined && value !== "").map(([key, value]) => `<div class="source"><strong>${esc(label(key))}</strong><div>${esc(value)}</div></div>`).join("")}</div><p><a href="/sources">Source, licence and methodology</a></p></details>`,
    {
